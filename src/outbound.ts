...
export function parseOutboundMediaMessage(text: string): ParsedOutboundMediaMessage {
  const files: OutboundMediaFile[] = [];
  const withoutDirectives = text.replace(MEDIA_DIRECTIVE_RE, (_match, _prefix: string, rawPath: string) => {
    const filePath = rawPath.trim();
    if (filePath) files.push(resolveOutboundMediaFile(filePath));
    return '';
  });
...
export async function uploadChatAttachment(params: {
  ...
  const response = await fetch(buildNextCompanyApiUrl(
    params.account,
    `/api/organizations/${encodeURIComponent(params.organizationId)}/chat/channels/${encodeURIComponent(params.channelId)}/messages/upload`,
  ), {
    method: 'POST',
    headers: {
...
function resolveOutboundMediaFile(filePath: string): OutboundMediaFile {
  const stat = statSync(filePath);
  if (!stat.isFile()) throw new Error(`Outbound media path is not a file: ${filePath}`);
...
function cleanupCaptionText(value: string): string {
  return value
    .replace(/[ \t]+\r?\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
